...
"****D echomsg '****' string(a:baseDir) string(l:absoluteFilespec) string(l:currentDirspec)
    ...
    let l:current  = l:currentDirspec
...
function! FilePathConvert#Do( text )
    ...
    if l:type ==# 'rel'
	return FilePathConvert#RelativeToAbsolute(s:GetBaseDir(), a:text)
    elseif l:type ==# 'abs'
	return FilePathConvert#AbsoluteToRelative(s:GetBaseDir(), a:text, ingofile#PathSeparator())
    elseif l:type ==# 'url'
